...
@Component
export default class FederationInput extends Vue {
  ...
  private items: Federation[] = [];
  private search: Federation | null = null;
  private select: number | null = null;
  @Prop() private value!: Federation;
  ...
  public async searchChange(query?: string) {
    ...
    this.isLoading = true;
    const url = this.restricted ? `my/federation?q=${q}` : `federation?q=${q}`;

    return API.get<Pagination<Federation>>(Pagination, url)
      .then(({result}) => {
        this.items = result;
      })
